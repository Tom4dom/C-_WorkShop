...
int[,] ElementFrequencyDictionaryCreator(int[] array)
{
    int[,] elementFreqTable = new int[array.Length, 2];
    int minValue = array[0];
    elementFreqTable[0, 0] = minValue;
    elementFreqTable[0, 1] = 1;
    int row = 0;
    ...
    for (int i = 1; i < array.Length; i++)
    {
        if (array[i] == minValue)
        {
            elementFreqTable[row, 1]++;
        }
        else
        {
            row++;
            elementFreqTable[row, 0] = array[i];
            elementFreqTable[row, 1]++;
            minValue = array[i];
        }
    }
    return elementFreqTable;
}
...
int[,] dictionaryMatrix = ElementFrequencyDictionaryCreator(result);
Console.WriteLine();
Console.WriteLine();
ArrayPrinter(dictionaryMatrix);
